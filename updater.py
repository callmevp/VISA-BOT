...
"""
This module is used for updating VISA
"""
import heroku3
import asyncio
import sys
...
GIT_REPO_NAME = "VISA-SPAM-BOT"
UPSTREAM_REPO_URL = "https://github.com/callmevp/VISA-SPAM-BOT"
...
ch_log += f"**# {
  ...
  UPSTREAM_REPO_URL.rstrip('/')}/commit/ {
  c
}) by ** {
  c.author
}**\n"
...
if conf != "now":
await ups.edit(
f"**Unfortunately, the directory {
  error
} does not seem to be a git repository.\
                \nOr Maybe it just needs a sync verification with {
  GIT_REPO_NAME
}\
...
if ac_br != "main":
await ups.edit(
f"**[UPDATER]:**` Looks like you are using your own custom branch ( {
  ac_br
}). "
  "in that case, Updater is unable to identify "
  "which branch is to be merged. "
  "Please checkout the official branch of VISA`"
)
repo.__del__()
return
...
changelog = await gen_chlog(repo, f"HEAD..upstream/ {
  ac_br
}")
...
await ups.respond(f"Do ` {
  xxxx
}update now` to update")
return
...
args = [sys.executable, "-m", "callmevp.py"]
